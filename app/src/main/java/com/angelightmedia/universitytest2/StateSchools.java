...
public class StateSchools extends AppCompatActivity {

    FragmentManager myfragManager;
    StateSchActivityFilterList stateSchActivityFilterList;
    StateSchActivitySchools stateschActivitySchoolsList;
    Spinner spinner;
    SearchView mySearchview;
    ListView myListView;
    SimpleCursorAdapter arrayAdapter;
    ArrayList<String> schoolslist;
    Button bt_home_state;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_state_schools);
        spinner = (Spinner)findViewById(R.id.spinner_stateschActivity);
        bt_home_state = findViewById(R.id.bt_home_stateSchActivity);
        myfragManager = this.getSupportFragmentManager();
        stateSchActivityFilterList = (StateSchActivityFilterList) myfragManager.findFragmentById(R.id.frag_sortby_stateschActivity);
        stateschActivitySchoolsList = (StateSchActivitySchools) myfragManager.findFragmentById(R.id.frag_schoolList_stateActivity);

        myfragManager.beginTransaction().
                show(stateSchActivityFilterList).commit();
        myfragManager.beginTransaction().
                show(stateschActivitySchoolsList).commit();
        mySearchview = findViewById(R.id.bt_search_stateschActivity);
        myListView = findViewById(R.id.searchForLVstate);
        useSearchview();

        bt_home_state.setOnClickListener(new View.OnClickListener() {
            @Override
